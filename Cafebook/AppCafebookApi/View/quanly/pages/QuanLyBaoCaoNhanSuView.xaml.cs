...
{
    // Thêm giao thức INotifyPropertyChanged để WPF tự động render lại UI
    public partial class QuanLyBaoCaoNhanSuView : Page, INotifyPropertyChanged
    {
        ...
        private async void Page_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (!AuthService.CoQuyen("FULL_ADMIN", "FULL_QL") && !AuthService.CoQuyen("QL_BAO_CAO_NHAN_SU"))
            {
            ...
            {
                MessageBox.Show($"Lỗi tải biểu đồ nhân sự: {ex.Message}");
            }
        }

        private void ApplyPermissions()
        {
            bool canExport = AuthService.CoQuyen("FULL_ADMIN", "FULL_QL") || AuthService.CoQuyen("QL_BAO_CAO_NHAN_SU");
        ...
        private async void BtnGenerate_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    // Lọc JSON bất chấp chữ hoa chữ thường
                    var options = new System.Text.Json.JsonSerializerOptions { PropertyNameCaseInsensitive = true };
                    currentReportData = await response.Content.ReadFromJsonAsync<QuanLyBaoCaoNhanSuTongHopDto>(options);
        ...
        private void PopulateUi(QuanLyBaoCaoNhanSuTongHopDto data)
        {
            ...
            // 3. Cập nhật Biểu đồ an toàn tránh lỗi Crash ép kiểu
            if (LuongChartSeries.Count > 0 && LuongChartSeries[0].Values is ChartValues<decimal> currentValues)
            {
                currentValues.Clear();
                currentValues.AddRange(data.LuongChartData.Select(d => d.TongTien));
            }

            // Kích hoạt INotifyPropertyChanged
            ChartLabels = data.LuongChartData.Select(d => "Tháng " + d.Ngay.ToString("MM/yyyy")).ToArray();
        }
